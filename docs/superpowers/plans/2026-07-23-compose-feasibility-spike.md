...
- **Build (5 source sets):** `cmd.exe /c "cd /d H:\\Repo\\RedstoneSpecs && gradlew.bat :26.1:clientClasses :26.1:classes :26.1:gametestClasses :26.1:clientTestClasses :26.1:testClasses"` (≤600000ms). **Runtime:** `runClientTest` (screenshot via `ViewportState.compositeCaptureRequest`) or a manual `runClient`.
