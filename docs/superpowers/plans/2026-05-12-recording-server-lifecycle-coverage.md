...
**Architecture:** Single new file `src/gametest/kotlin/.../test/recorder/RecordingLifecycleSpec.kt` extending `RedstoneTestSpec`, mirroring the style of `RecorderRunnerNetworkRegistrySpec`. Each test embeds the UC ID in its name. Async file-write tests use a small inline poll-with-timeout helper. Marker/UndoStack tests are pure unit tests inside the same Kotest spec (no `onServer`).

**Tech Stack:** Kotest (gametest source set), Fabric, Kotlin coroutines, existing helpers (`RedstoneTestSpec`, `withTempRoot`, `placeRecorderBE`, `makeMockServerPlayer`, `drainPayloads`, `onServer`).
...
**Spec registration (per project memory):** Every new Kotest spec MUST be added to the explicit `specs = listOf(...)` list in `src/gametest/kotlin/com/breadmoirai/redstonespecs/test/GametestSentinel.kt`. Autoscan is off — unregistered specs silently don't run.

**Test-thread model (per project memory):** `RedstoneTestSpec` test bodies run on a worker. Code that touches the world / BE must be wrapped in `onServer { ... }` (a helper that hops to `McDispatchers.Server`). Pure unit tests don't need it.
...
**Files:**
- Create: `src/gametest/kotlin/com/breadmoirai/redstonespecs/test/recorder/RecordingLifecycleSpec.kt`
...
In `src/gametest/kotlin/com/breadmoirai/redstonespecs/test/GametestSentinel.kt`, add the import next to the other `test.*` imports:
...
`SpecBlockEntity` lives in `com.breadmoirai.redstonespecs.block`. The default state is `ModRegistries.REDSTONE_SPEC_RECORDER_BLOCK.defaultBlockState()`.
...
Add inside the `RedstoneTestSpec({ ... })` body:

```kotlin
test("UC-REC-03.c: isConfigured returns false for blank specId") {
    val be = com.breadmoirai.redstonespecs.block.SpecBlockEntity(
        net.minecraft.core.BlockPos.ZERO,
        com.breadmoirai.redstonespecs.ModRegistries.REDSTONE_SPEC_RECORDER_BLOCK.defaultBlockState(),
    )
    be.setSpecId("")
...
test("UC-REC-04.a/c: startRecording succeeds and recorder appears in activeRecorders") {
    onServer {
        ...
        try {
            ...
            val active = com.breadmoirai.redstonespecs.runner.StateRecorder.activeRecorders()
    ...
    onServer {
        ...
        try {
            ...
            val recorder = be.javaClass.getDeclaredField("stateRecorder")
                ...
                .get(be) as com.breadmoirai.redstonespecs.runner.StateRecorder
...
test("UC-REC-05.a: stopRecordingAndFinalize deactivates recorder and clears stateRecorder") {
    onServer {
        ...
        be.startRecording() shouldBe true
        val activeBefore = com.breadmoirai.redstonespecs.runner.StateRecorder.activeRecorders().size

        be.stopRecordingAndFinalize() shouldBe true
        be.isRecording shouldBe false
        com.breadmoirai.redstonespecs.runner.StateRecorder.activeRecorders().size shouldBe (activeBefore - 1)
    }
}
```
...
test("UC-REC-05.e: stopRecordingAndFinalize with markers writes .spec.kts under SharedSettings.specSaveDir") {
    ...
    val expectedPath = onServer {
        ...
        be.addOrUpdateMarker(
            com.breadmoirai.redstonespecs.runner.EntryMarker(
                pos = net.minecraft.core.BlockPos(1, 0, 0),
                label = "input_a",
                color = 0xFF4488FF.toInt(),
                kind = com.breadmoirai.redstonespecs.runner.EntryMarker.Kind.INPUT,
            )
        )
        be.startRecording() shouldBe true
        be.stopRecordingAndFinalize() shouldBe true
        this.getWorldPath(net.minecraft.world.level.storage.LevelResource.ROOT)
            .resolve(com.breadmoirai.redstonespecs.config.SharedSettings.specSaveDir)
...
test("UC-REC-06.a: handleRecorderCommand(DISCARD) calls discardForRerecord (does not touch active recorder)") {
    onServer {
        ...
        val marker = com.breadmoirai.redstonespecs.runner.EntryMarker(
            pos = net.minecraft.core.BlockPos(1, 0, 0),
            label = "input_a",
            color = 0xFF4488FF.toInt(),
            kind = com.breadmoirai.redstonespecs.runner.EntryMarker.Kind.INPUT,
        ...
        com.breadmoirai.redstonespecs.network.handleRecorderCommand(
            this, player,
            com.breadmoirai.redstonespecs.network.RecorderCommandC2S(
                pos, com.breadmoirai.redstonespecs.network.RecorderCmd.DISCARD,
            ),
        )
...
test("UC-REC-06.d: discardForRerecord collapses duplicate (pos,kind) entries to one each") {
    onServer {
        ...
        val rel = net.minecraft.core.BlockPos(1, 0, 0)
        val a = com.breadmoirai.redstonespecs.runner.EntryMarker(
        ...
        val cOut = a.copy(
            label = "output_a",
            kind = com.breadmoirai.redstonespecs.runner.EntryMarker.Kind.OUTPUT,
        )
        ...
        byKind[com.breadmoirai.redstonespecs.runner.EntryMarker.Kind.INPUT]?.size shouldBe 1
        byKind[com.breadmoirai.redstonespecs.runner.EntryMarker.Kind.OUTPUT]?.size shouldBe 1
    }
}
```
...
test("UC-REC-02.c: InputSpecMarkerItem.createMarker yields input_a then input_b with color 0xFF4488FF") {
    ...
    val item = com.breadmoirai.redstonespecs.item.InputSpecMarkerItem()
    ...
    first.kind shouldBe com.breadmoirai.redstonespecs.runner.EntryMarker.Kind.INPUT
...
test("UC-REC-02.c: OutputSpecMarkerItem.createMarker uses color 0xFFFF8800") {
    ...
    val marker = com.breadmoirai.redstonespecs.item.OutputSpecMarkerItem()
        .createMarker(net.minecraft.core.BlockPos(1, 0, 0), be)
    marker.label shouldBe "output_a"
    marker.color shouldBe 0xFFFF8800.toInt()
    marker.kind shouldBe com.breadmoirai.redstonespecs.runner.EntryMarker.Kind.OUTPUT
}
```
...
test("UC-REC-02.e: UndoStack push then pop returns the marker; cap at 20 per UUID") {
    ...
    val mk = { i: Int ->
        com.breadmoirai.redstonespecs.item.UndoStack.UndoRecord(
            originPos = origin,
            marker = com.breadmoirai.redstonespecs.runner.EntryMarker(
    ...
    com.breadmoirai.redstonespecs.item.UndoStack.clear(uuid)

    val r0 = mk(0)
    com.breadmoirai.redstonespecs.item.UndoStack.push(uuid, r0)
    com.breadmoirai.redstonespecs.item.UndoStack.pop(uuid) shouldBe r0

    // Push 21; bottom must have been evicted -> popping 20 times yields entries 1..20 reversed.
    for (i in 0..20) com.breadmoirai.redstonespecs.item.UndoStack.push(uuid, mk(i))
    val popped = generateSequence { com.breadmoirai.redstonespecs.item.UndoStack.pop(uuid) }.toList()
...
Read the Kotest XML report under `build/reports/redstonespecs/gametest/` (per `GametestSentinel.runAll`'s `reportsDir`). Common failure modes:
