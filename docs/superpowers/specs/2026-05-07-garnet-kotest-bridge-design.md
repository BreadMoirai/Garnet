---
title: GarnetSpec ↔ Kotest bridge
tags: [design, runner, kotest, dsl, persistence, lifecycle]
summary: Make authoring and running an in-game GarnetSpec the same shape as writing and running a Kotest unit test, by shipping kotest-engine + kotlin-scripting in main and replacing the SpecEntry data model with executable .spec.kts files.
---

# GarnetSpec ↔ Kotest bridge — Design

## Goal

When a user defines a `GarnetSpec` in-game, the development process and the end result should be indistinguishable from writing and running a Kotlin unit test:
...
- Single base class `GarnetTestSpec` used by both shipped and dev-side tests.
...
execution:   load .kts → kotest engine → test body drives runner → assertions fire
             KtsSpecLoader    GarnetTestEngine    RunnerDsl       Kotest TestResult
...
New package: `net.breadmoirai.garnet.testing` in the `main` source set.

| Component | Responsibility | Notes |
|---|---|---|
| `GarnetTestSpec` | Base class extending Kotest `FunSpec`, wraps lifecycle in `withContext(McDispatchers.Server)` via `CoroutineDispatcherFactory`. | Used by both shipped `.spec.kts` and dev tests in `src/clientTest/`, `src/gametest/`, `src/test/`. Replaces the dev-only `ServerTestSpec`. |
| `GarnetTestEngine` | Programmatic Kotest launcher. Single entry point for in-game Run buttons, `GarnetRunnerBlock`, and dev-side gradle tasks. | Wraps Kotest's `KotestEngineLauncher`. Configured programmatically (no `ServiceLoader`) to sidestep Fabric classloader issues. |
...
- The dev-only `ServerTestSpec` class (collapsed into `GarnetTestSpec`).
...
- `SpecEntry` and the per-entry data shape — still consumed by `GarnetSpec.entries`,
...
```kotlin
class ComparatorLatchSpec : GarnetTestSpec({
    test("comparator latches after 4 ticks") {
        val s = spawnStructure(id("garnet", "comparator_basic"))
...
1. User clicks Run on a `GarnetRunnerBlock`.
2. Server resolves spec id → `<id>.spec.kts` text.
3. `KtsSpecLoader.compile(text)` → `Spec` class (cached by content hash).
4. `GarnetTestEngine.run(spec)` launches Kotest on a worker thread; `CoroutineDispatcherFactory` ensures test-body code runs on the server thread.
...
7. HTML/JUnit reports written to disk under `build/reports/garnet/runtime/` for parity with dev tests.

## Convergence with dev-side tests

After this change:

- All test source sets (`src/test/`, `src/gametest/`, `src/clientTest/`) extend `GarnetTestSpec` instead of the now-removed `ServerTestSpec`.
...
The `docs/gametest/kotest-bridge.md` article continues to apply to dev tests verbatim — `awaitTicks`, `onServer`, `spawnStructure` semantics are unchanged. Update its base-class reference from `ServerTestSpec` to `GarnetTestSpec` and note that the same primitives are now also available to shipped specs.
...
- The bridge itself (`KtsSpecLoader`, `RecordingToKts`, `GarnetTestEngine`) gets dev-side Kotest specs in `src/test/` (no MC needed) and `src/clientTest/` (end-to-end with a real world).
...
- HTML report destination for runtime runs — same `build/reports/garnet/runtime/` for dev parity, or a world-save-relative path?
