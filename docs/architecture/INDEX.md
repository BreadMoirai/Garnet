...
- [Module map](module-map.md) — Tour of the source tree (`core/`, `playback/`, `testing/`, `editor/` and its five sub-features, `dock/`, `mixin/`), which package owns what, the dependency direction, and where to start reading for a given concern. Tags: modules, layout, dependencies, entry-points.
