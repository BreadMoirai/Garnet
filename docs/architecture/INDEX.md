...
- [Module map](module-map.md) — Tour of the source tree (`spec/`, `core/async/`, `core/events/`, `structure/`, `config/`, `ui/`, `playback/`, `testing/`, `editor/`), package responsibilities, dependency direction, and where to start reading for a given concern. Tags: modules, layout, dependencies, entry-points.
