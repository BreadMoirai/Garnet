...
- **UC-MAN-10.e** All three handlers reply with `StructureResultS2C(subpath, sizeX, sizeY, sizeZ, hasUnsaved, message)`; `ProjectClientNetworking` feeds it to `ProjectTreeState.onStructureResult`, which sets `status = message` — the same status line used by folder load/save results. As of the packet-shape change, `hasUnsaved` is wired to `false` at both call sites (`handlePlaceStructure`, `handleSaveStructure`) pending the dirty-tracking logic that will populate it. A `DiscardStructureC2S(subpath)` packet also now exists (mirrors `SaveStructureC2S`) but has no registered handler yet.
