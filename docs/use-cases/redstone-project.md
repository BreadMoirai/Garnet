...
- **UC-MAN-07.f** `EditorSaveReportS2C` is sent back to the player with per-spec `"specId|saved=true/false[|err=…]"` strings; `StructureInfoState.onSaveReport` sets `status = "saved N spec(s)"`, which the [Structure Info panel](../ui/structure-info-panel.md) renders as its status line (the same mechanism `ProjectScreen.onSaveReport` used before deletion).
...
| UC-MAN-09 | Re-root the Explorer from a native folder picker | `RootPickerControllerTest`, `EditorNetworkRegistrySpec` | **GAP-PARTIAL** |
| UC-MAN-09.a | Kebab menu toggles the menu; Open Folder runs the `FolderPicker` on a worker thread | `RootPickerControllerTest."openFolder sends SetEditorRootC2S and persists the picked path"`, `JewelExplorerSpec."the Explorer tree renders, selects, navigates, and its kebab menu never outlives a remount"` | covered |
| UC-MAN-09.b | Non-null pick persists + sends `SetEditorRootC2S`; cancel sends nothing | `RootPickerControllerTest."openFolder sends SetEditorRootC2S and persists the picked path"`, `RootPickerControllerTest."openFolder sends nothing when the picker is cancelled"` | covered |
...
| UC-MAN-11 | Rejoin a world and find the tree already loaded, with last session's expansion restored | `ExplorerStateStoreTest`, `ExplorerTreeStateTest`, `ExplorerLifecycleTest` | covered |
| UC-MAN-11.a | `JOIN` handler arms the restore and sends `ListEditorTreeC2S` (guarded by `canSend`); the singleplayer gate is two-sided on both save and arm | `ExplorerLifecycleTest."singleplayer with a snapshot present writes the record"`, `ExplorerLifecycleTest."not singleplayer writes nothing"`, `ExplorerLifecycleTest."not singleplayer arms no restore"`, `ExplorerLifecycleTest."singleplayer arms a restore"` | covered |
| UC-MAN-11.b | Snapshot dispatch applies the pending restore; a root mismatch discards it | `ExplorerTreeStateTest."an armed restore reopens the persisted folders when the snapshot lands"`, `ExplorerTreeStateTest."a restore captured against a different root is discarded"` | covered |
| UC-MAN-11.c | Stale/renamed paths dropped; only folders are restored to `openNodes` | `ExplorerTreeStateTest."paths that no longer exist are dropped from the restore"`, `ExplorerTreeStateTest."a file path is never restored as an expanded node"` | covered |
| UC-MAN-11.d | Restore is one-shot; `reset()` disarms a pending restore | `ExplorerTreeStateTest."the restore is one-shot: a second snapshot does not clobber live expansion"`, `ExplorerTreeStateTest."reset disarms a pending restore"` | covered |
| UC-MAN-11.e | `ExplorerStateStore` round-trips `garnet-explorer.json`; blank root / no-snapshot skip writing | `ExplorerStateStoreTest."a session round-trips through garnet-explorer.json"`, `ExplorerStateStoreTest."a null selection round-trips as null rather than an empty string"`, `ExplorerStateStoreTest."a missing file loads as null"`, `ExplorerStateStoreTest."a malformed file loads as null instead of throwing"`, `ExplorerStateStoreTest."a record with no root loads as null"`, `ExplorerStateStoreTest."saving with a blank root writes nothing"` | covered |
