...
- **UC-MAN-10.h** `RenamePathC2S(subpath, newName)` (Explorer's right-click "Rename") → `handleRename`: resolves `subpath` via `ProjectRoot.resolveSubpath` and refuses `subpath == ""` (the client disables the menu item for the root, but the server does not trust that), re-validates `newName` via `ProjectNames.validate` against the parent's real sibling names (excluding the node's own current name, so re-committing an unchanged name is a no-op rather than a self-collision). If the renamed node is a currently-placed structure (`ProjectDimRegistry.placedBoxOf(subpath) != null`), it clears that footprint (`StructurePersistence.clearBounds`) and calls the new `ProjectDimRegistry.unplaceStructure(subpath)` — dropping both the `placedBoxes` and `structureBySubpath` entries — *before* the file move, then re-places it under the new subpath via `placeStructureFrom` afterwards. The structure lands in a freshly-assigned region (`nextStructureIndex` is monotonic and never recycled) rather than reusing the old one — this is intended, matching how every other region assignment in the registry behaves. A rename of a structure that is *not* placed skips all of that and only moves the file. Any `<name>.nbt.unsaved` sidecar (`StructurePersistence.unsavedSidecarOf`) is moved alongside the `.nbt` so unsaved edits stay attached to the renamed structure. If `ProjectSession.activeSubpath` equals the renamed subpath, or is nested under it, the session is repointed onto the new subpath (`repointSession`, boundary-safe via a `"$oldSubpath/"` prefix match — renaming `redstone` repoints `redstone/clocks` but not an unrelated sibling like `redstoneworks/clocks`). On success the tree is re-sent; any failure (name collision, filesystem error) replies with `ProjectErrorS2C` and leaves the source untouched.
