...
- **UC-MAN-08.b** After unload, the player's `activeSubpath` is `null`; a subsequent `handleNewSpec` (which requires a folder focus) receives `ManagedErrorS2C("no folder selected")`. `handleSaveNow` is deliberately session-independent — it calls `ManagedDimLifecycle.saveAll(server)` over every loaded folder in `ManagedWorld.perFolder`, so post-unload it still returns a `ManagedSaveReportS2C` (empty when nothing is loaded), not an error.
...
| UC-MAN-03.e | `ManagedDimRegistry` keyed by `MinecraftServer`; `dispose` called on server stop | `ManagedDimRegistryTest."regionOriginOf returns null for unassigned subpath"`, `ManagedLifecycleReleaseTest."UC-MAN-08.d: releaseServerState disposes registry, world, and context"` | **GAP-PARTIAL** |
...
| UC-MAN-08.b | Post-unload `handleNewSpec` returns `ManagedErrorS2C("no folder selected")`; `handleSaveNow` is session-independent (saves all loaded folders) | `ManagedNetworkRegistrySpec."handleNewSpec without active session returns 'no folder selected'"` | covered |
