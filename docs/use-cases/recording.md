...
last_audited_commit: 860d64dea38249834d05f5ab3974b39f99c850d3
---
...
**Outcome:** The in-memory `StateRecording` is converted to `.spec.kts` DSL source text by `RecordingDslEmitter.emit` and written to disk under `SharedSettings.specSaveDir` (or, in project contexts, to `projectSourcePath`). `SpecBlockEntity.isRecording` becomes `false`.
...
- UC-REC-05.e — The resulting DSL source text is written asynchronously (via `Dispatchers.IO` coroutine on the `SpecBlockEntity`'s `coroutineScope`) using `SpecPersistence.writeSpecKts` for normal worlds, or directly via `projectSourcePath.writeText` for project-dimension contexts.
...
| UC-REC-05.e | DSL source written async via `SpecPersistence.writeSpecKts` or `projectSourcePath.writeText` | `RecordingLifecycleSpec."UC-REC-05.e: stopRecordingAndFinalize with markers writes .spec.kts under SharedSettings.specSaveDir"`, `RecordingLifecycleSpec."UC-REC-05.e: projectSourcePath redirects write to that path instead of saveDir"`, `SpecPersistenceTest."writeSpecKts then load round-trips a new-dsl spec"` | covered |
