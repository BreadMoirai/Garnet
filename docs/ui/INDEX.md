...
- [Jewel — the dock's widget layer](jewel-widget-layer.md) — The dock's IntelliJ-look widget layer: the load-bearing Jewel/Compose/skiko triple, why icons need a separate artwork artifact, why in-scene popups need the dock's mount-lifecycle guards, the Jewel-authoritative tree-state model, and the right-click `New`/`Rename` context menu (row-local→window coordinate conversion, `ExplorerActions` validate-then-send seam). _[compose, jewel, dock, icons, popup, skiko, versions]_
