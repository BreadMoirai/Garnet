...
- [Local history for standalone structures](local-history.md) — How auto-saved .nbt structures record revisions under <instance>/.garnet/local-history, why the key is the file's absolute path, how pruning works, and why a deleted structure keeps its history (so a file recreated at the same path inherits it). Tags: storage, history, autosave, structures, persistence
