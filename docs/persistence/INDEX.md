...
- [Editor C2S/S2C payload contract](network-payload-contract.md) — Server-only authority for the editor/Explorer wire protocol: path-containment via `EditorRoot.resolveSubpath`, per-player intent via `EditorSession`, `CameraModeC2S`'s elevated `/gamemode` gated on a server-written grant flag, replacing the deleted block-entity/`originPos` trust anchor. Tags: networking, payloads, sync, authority, editor
