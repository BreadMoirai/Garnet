...
`RedstoneSpec::class.java.classLoader`. Without this pin the script's
RedstoneSpec can come from the system classloader while the host's
RedstoneSpec comes from Fabric's mod ("knot") classloader, and the cast
`rv.value as RedstoneSpec` fails at runtime even though the class file
...
`RedstoneSpec`. The standard form is:
