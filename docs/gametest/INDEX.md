...
- [Kotest + coroutine test bridge](kotest-bridge.md) — How specs work; the awaitTicks/onServer/spawnStructure cookbook; the `GametestSentinel` gotchas (no classpath discovery, never resolve a gametest from a server task, orphaned world locks); invariants. Tags: testing, kotest, coroutines.
...
- [Screenshots for debugging and visual regression](screenshots-for-debug-and-regression.md) — `takeClientScreenshot` for diagnostic capture today, how to wire Fabric's `TestScreenshotComparisonOptions` for committed-baseline visual regression when we want it, and `PanelPixelProbe` for asserting on the dock composite's actual pixels. _Tags: testing, client-gametest, screenshots, debugging, regression, pixel-probe_
