...
suspend fun awaitTickEnd(): MinecraftServer = ServerTickFlows.serverTickEnd.first()
...
suspend fun awaitTicks(n: Int): MinecraftServer {
    ...
    return ServerTickFlows.serverTickEnd.take(n).last()
}
...
suspend fun awaitTickWhere(predicate: (MinecraftServer) -> Boolean): MinecraftServer =
    ServerTickFlows.serverTickEnd.first(predicate)

/** Hops to the server thread, runs [block], returns the result. */
suspend fun <T> onServer(block: suspend MinecraftServer.() -> T): T =
    withContext(AsyncDispatchers.Server) { block(AsyncDispatchers.currentServer) }
