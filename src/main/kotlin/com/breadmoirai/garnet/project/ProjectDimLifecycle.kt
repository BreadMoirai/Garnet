...
object ProjectDimLifecycle {
     ...
     * in-memory ephemeral). Lazily creates the [ProjectWorld] for `server` if not already set.
     */
    fun placeAll(server: MinecraftServer, root: ProjectRoot): List<LoadFolderReport> {
        val world = ProjectWorld.get(server)?.takeIf { it.root == root }
            ?: ProjectWorld(root).also { ProjectWorld.set(server, it) }
     ...
     * after a `ProjectNewSpec.create` to materialize the new cell.
     */
    fun placeFolder(server: MinecraftServer, root: ProjectRoot, subpath: String): LoadFolderReport {
        val world = ProjectWorld.get(server)?.takeIf { it.root == root }
            ?: ProjectWorld(root).also { ProjectWorld.set(server, it) }
    ...
    private fun placeFolderInto(
        ...
        LOGGER.info(
            "[ProjectDimLifecycle#placeFolder] placed folder '{}' ({} specs, {} layout errors, {} parse errors)",
     ...
     * Per-player [ProjectSession] entries are not touched here — they are released on each
    ...
    fun saveFolder(server: MinecraftServer, subpath: String): List<CellSaveResult> {
        ...
        for ((id, loaded) in perFolder) {
            ...
            val r = ProjectCellSaver.captureAndSaveIfDirty(level, loaded, abs, folderAbsolute)
            results.add(r)
            if (r.saved) {
