 ...
 * cell snapshots. Created by [ProjectDimLifecycle.placeAll] on server start; persists for the
 * server's lifetime.
 *
...
class ProjectWorld(
    ...
    fun absoluteCellOrigin(server: MinecraftServer, subpath: String, specId: String): BlockPos? {
        ...
        val region = ProjectDimRegistry.of(server).regionOriginOf(subpath) ?: return null
        val rel = loaded.cell.origin
        return BlockPos(region.x + rel.x, rel.y, region.z + rel.z)
    }

    companion object {
        private val perServer = java.util.WeakHashMap<MinecraftServer, ProjectWorld>()
        @Synchronized fun set(server: MinecraftServer, world: ProjectWorld) { perServer[server] = world }
        @Synchronized fun get(server: MinecraftServer): ProjectWorld? = perServer[server]
