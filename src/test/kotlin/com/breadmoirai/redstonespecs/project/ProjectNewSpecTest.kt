...
class ProjectNewSpecTest : FunSpec({
    ...
    test("illegal characters in name throw") {
        ...
        try {
            shouldThrow<IllegalArgumentException> { ProjectNewSpec.create(tmp, "with space") }
            shouldThrow<IllegalArgumentException> { ProjectNewSpec.create(tmp, "a/b") }
            shouldThrow<IllegalArgumentException> { ProjectNewSpec.create(tmp, "dot.name") }
        } finally {
            Files.walk(tmp).use { it.sorted(Comparator.reverseOrder()).forEach(Files::delete) }
        }
    }
    ...
    test("file already exists throws") {
        ...
        try {
            ...
            val ex = shouldThrow<IllegalArgumentException> { ProjectNewSpec.create(tmp, "dup") }
    ...
    test("create writes <name>.spec.kts with stub content") {
        ...
        try {
            val out = ProjectNewSpec.create(tmp, "fresh")
