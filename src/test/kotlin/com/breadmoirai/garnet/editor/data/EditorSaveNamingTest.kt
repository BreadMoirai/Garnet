...
class EditorSaveNamingTest : FunSpec({

    test("preserves alphanumeric tail and appends 8-hex hash") {
        val name = EditorSaveNaming.saveName(Path.of("/foo/specs"))
    ...
    test("same absolute path produces same name across calls") {
        ...
        EditorSaveNaming.saveName(p) shouldBe EditorSaveNaming.saveName(p)
    }

    test("hash suffix is exactly 8 hex chars") {
        val name = EditorSaveNaming.saveName(Path.of("/x/y/z"))
