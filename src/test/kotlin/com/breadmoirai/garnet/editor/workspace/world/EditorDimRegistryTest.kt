...
class EditorDimRegistryTest : FunSpec({

    fun newRegistry(): EditorDimRegistry =
        EditorDimRegistry(Mockito.mock(MinecraftServer::class.java))
    ...
    test("region width matches cellSize.x * rowMax + gap*(rowMax+1) + REGION_PAD") {
        ...
        val expectedWidth = cs.x * rowMax + gap * (rowMax + 1) + EditorDimRegistry.REGION_PAD
    ...
    test("structure regions sit in a lane disjoint from spec-folder regions") {
        ...
        spec.z shouldBe 0
        struct.z shouldBe EditorDimRegistry.STRUCTURE_LANE_Z
    }
