...
class RecordingDslEmitterTest : FunSpec({
    ...
    test("emits garnetSpec header with correct metadata") {
        ...
        source shouldContain "garnetSpec("
    ...
    test("emits structure parameter when provided") {
        val source = RecordingDslEmitter.emit(
            ...
            bounds = Vec3i(3, 3, 3),
            lifespan = 4,
            structure = "garnet:my_structure",
            strict = false,
            markers = buildMarkers(),
            recording = buildRecording(),
        )

        source shouldContain "structure = \"garnet:my_structure\""
    }
