...
object BlitUvPipeline {
    ...
    private const val NAMESPACE = "garnet"
    ...
    fun blit(from: GpuTextureView, to: RenderTarget, x1: Float, y1: Float, x2: Float, y2: Float, flipV: Boolean = false, blend: Boolean = false) {
        ...
        device.createBuffer({ "garnet blit_uv vertices" }, GpuBuffer.USAGE_VERTEX, vertexData).use { vertexBuffer ->
            device.createCommandEncoder()
                .createRenderPass({ "garnet blit_uv" }, target, Optional.empty<Vector4fc>())
                .use { pass ->
