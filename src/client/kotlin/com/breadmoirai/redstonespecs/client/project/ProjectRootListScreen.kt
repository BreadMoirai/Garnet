 ...
 * from the main menu via `TitleScreenMixin` ("Project Specs..." button).
 *
 * Reads/writes via [ProjectRootsConfig] under the MC config dir. Picking a root will (in
 * T21) boot the integrated server pinned to that root; for now we log + close.
 */
class ProjectRootListScreen(private val parent: Screen) :
    Screen(Component.literal("Redstone Project Roots")) {

    private val configPath: Path =
        FabricLoader.getInstance().configDir.resolve("redstonespecs/project-roots.json")
    private var roots: MutableList<String> = ProjectRootsConfig.load(configPath).toMutableList()
    ...
    override fun init() {
        ...
        outer.addChild(StringWidget(Component.literal("Redstone Project Roots"), font))
            ...
            roots.toList().forEachIndexed { idx, r ->
                ...
                row.addChild(
                    Button.builder(Component.literal("X")) {
                        roots.removeAt(idx)
                        ProjectRootsConfig.save(configPath, roots)
                        rebuildWidgets()
    ...
    private fun openRoot(rootPath: String) {
        ...
        ProjectIntegratedBoot.boot(path)
